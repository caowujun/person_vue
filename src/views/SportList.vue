<template>
  <el-card class="box-card" shadow="hover">
    ...
    <el-table :data="tableData" style="width: 100%" border stripe size="small" v-loading="loading"
      ...
      <el-table-column :label="$t('sport.date')" prop="recorddate" :formatter="dateFormat">
      </el-table-column>
      <el-table-column :label="$t('sport.weight')" prop="weight">
      </el-table-column>
      <el-table-column :label="$t('sport.sex')" prop="sex">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDelete (index, row) {
      this.$local.confirm(() =>
        this.$http.post(this.$apiList.deletesport, this.$qs.stringify({
          id: row.id
        }))
    ...
    loadList () {
      this.$http.get(this.$apiList.sportpage, {
        params: {
